    ...
    <div className={`nav-menu-wrapper justify-end `}>
      <div
        className={`nav-menu w-full ${
          isLogin ? "mr-8 border-primary/70 pr-8  lg:border-r-[1px]" : ""
        }`}
      >
        {!isLogin ? (
        ...
        <div className="flex items-center gap-4 md:gap-8">
          <div className="relative flex gap-4">
            <Button isLink path="/cart" className={""}>
              ...
              {total > 0 && (
                <span
                  className={`absolute -right-2 -top-2 flex h-[18px] w-[18px] items-center justify-center rounded-full bg-danger text-[10px] text-white`}
                >
                  {total}
                </span>
              )}
          ...
          <div
            className={`profile-img-wrapper relative row-start-2 flex w-full items-center gap-2 ${
              isAccountVerified
                ? "lg:rounded-full lg:border lg:border-danger lg:p-0.5"
                : null
            }`}
            onMouseOver={() => setIsMenuVisible(true)}
            onMouseLeave={() => setIsMenuVisible(false)}
          >
            <div className="nav-profile-img hidden aspect-square w-8 cursor-pointer self-center overflow-hidden rounded-full border md:mb-0 lg:block">
              ...
              {ongoingTransactions > 0 && (
                <span className="absolute -right-2 top-0 flex h-4 w-4 items-center justify-center rounded-full bg-danger text-[12px] text-white group-hover:right-1">
                  {ongoingTransactions}
                </span>
              )}
            </div>
                  ...
                  <div className="rounded-lg border bg-slate-100 px-6 py-4 shadow-lg">
                    ...
                    <div className="flex flex-col gap-2 pt-4">
                      ...
                      >
                        ...
                        {ongoingTransactions > 0 && (
                          <span
                            className={`flex h-[18px] w-[18px] items-center justify-center rounded-full bg-danger text-[10px] text-white`}
                          >
                            {ongoingTransactions}
                          </span>
                        )}
